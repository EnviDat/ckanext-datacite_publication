...
REQUEST_MESSAGE = 'requested datacite publication'
APPROVAL_MESSAGE = 'approved datacite publication'
FINISH_MESSAGE = 'finished datacite publication'
UPDATE_MESSAGE = 'updated datacite publication'
...
def datacite_publication_requested_mail(user_id, entity, user_email='', entity_type='package'):
    try:
        log.debug('datacite_publication_requested_mail: Notifying request from "{0}" ({1})'.format(user_id,
                                                                                                   entity.get('name',
                                                                                                              '')))
    ...
    try:
        ...
        if user_request:
            ...
            if user_request['email'] not in sent_mails:
                mailer.mail_recipient(user_request.get('display_name', user_request['name']), user_request['email'],
                                      subject, body)
        ...
        if entity.get("maintainer_email") and entity.get("maintainer_email") not in sent_mails:
            mailer.mail_recipient("Dataset Contact Point", entity.get("maintainer_email"), subject,
                                  "\t ** COPY ** \n\n" + body)
        ...
        log.error(('datacite_approved_mail: '
                   'Failed to send mail to user "{0}": {1}, {2}').format(user_id, e,
...
def datacite_finished_mail(user_id, entity, context, user_email='', entity_type='package'):
    try:
        log.debug(
            'datacite_finished_mail: Notifying finishined publication of dataset {0}'.format(entity.get('name', '')))
...
def _get_user_request(entity_id, context):
    try:
        ...
        for revision in revisions_list:
            ...
            revision_message = revision.get('activity_type', '')
            if revision_message.lower().find(REQUEST_MESSAGE.lower()) >= 0:
                user_request_id = revision.get('user_id')
                user_request = toolkit.get_action('user_show')(context, {'id': user_request_id})
                log.debug("FOUND USER = {0}".format(user_request))
                return user_request
    except Exception as e:
...
def _add_activity(dataset_dict, message, context):
    ckan_user = _get_username_from_context(context)
    userobj = ckan.model.User.get(ckan_user)
    if userobj:
        request_activity = ckan.model.activity.Activity(
            userobj.id,
            dataset_dict['id'],
            message,
            {
